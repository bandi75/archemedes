...
**Version:** v2.2  
**Status:** Implementation-ready baseline — React frontend view-model update  
...
Archimedes is implemented as a FastAPI backend that exposes session, pipeline, artifact, evidence, Socrates, re-reasoning, streaming-event, and frontend view-model APIs. The APIs sit above the orchestrator, agent routines, Socrates workflow, Architecture State Manager, Cosmos DB repositories, Blob Storage, and Foundry IQ retrieval layer.
...
- Supporting frontend timeline, debate view, artifact viewer, Mermaid rendering view, evidence explorer, artifact package studio, live reasoning trace, and change-impact diff view.
...
9. **Frontend-friendly status APIs**  
   The UI should not reconstruct pipeline state by querying many raw collections. It should use dedicated timeline, status, artifact, and view-model APIs.

10. **View-model boundary for React**  
   The React/Next.js frontend should consume screen-ready read models for hero screens. It should not parse raw stage artifact JSON or stitch together many low-level collections for primary UI rendering.

11. **Snapshot-then-stream for live UX**  
   Long-running stage and Socrates execution should be rendered from a durable snapshot first, then kept current through event history and Server-Sent Events.

12. **Internal tools are not public APIs by default**  
...
| Group | Purpose |
|---|---|
| `/dashboard` | Cross-session dashboard summary for the React home / command-center screen |
| `/sessions` | Create, retrieve, list, and update architecture sessions |
| `/sessions/{session_id}/overview` | Session-level screen-ready summary for React headers and session landing views |
| `/sessions/{session_id}/pipeline` | Run stages, inspect pipeline timeline, pause/resume/retry |
| `/sessions/{session_id}/pipeline/view` | Screen-ready Pipeline view model with stage metrics, recent events, and selected-stage detail |
| `/sessions/{session_id}/stages` | Stage status and stage-run details |
| `/sessions/{session_id}/events` | Durable JSON event history for replay/catch-up |
| `/sessions/{session_id}/events/stream` | SSE stream for live orchestration and reasoning trace |
| `/sessions/{session_id}/requirements/view` | Screen-ready Requirements Review model |
| `/sessions/{session_id}/patterns/view` | Screen-ready Pattern Explorer model |
| `/sessions/{session_id}/options/view` | Screen-ready Architecture Options board model |
| `/sessions/{session_id}/socrates` | Run/retrieve Socrates review resource output |
| `/sessions/{session_id}/socrates/view` | Screen-ready Socrates Reasoning Lab model |
| `/sessions/{session_id}/artifacts` | Retrieve versioned stage artifacts |
| `/sessions/{session_id}/artifacts/package-view` | Screen-ready Artifact Studio / architecture package model |
| `/sessions/{session_id}/claims` | Retrieve, filter, and validate claims |
| `/sessions/{session_id}/evidence` | Retrieve evidence sources |
| `/sessions/{session_id}/evidence/view` | Screen-ready Evidence & Claims Explorer model |
...
| `/sessions/{session_id}/changes/{change_event_id}/impact-view` | Screen-ready Change Impact Studio model |
| `/sessions/{session_id}/diffs` | Retrieve artifact diffs |
| `/sessions/{session_id}/viewer/*` | Legacy/development artifact viewer helpers; not the primary React contract |
| `/health` | Health and readiness |
| `/admin` | Optional dev/admin endpoints |

### 7.1 Resource APIs vs View-Model APIs

The API surface intentionally contains two layers:

| Layer | Purpose | Consumers |
|---|---|---|
| Resource APIs | CRUD-like access to sessions, stage runs, artifacts, claims, evidence, changes, and diffs | Backend services, admin tooling, debugging, power users |
| View-model APIs | Screen-ready read models that aggregate multiple resources for React | React/Next.js frontend hero screens |

Rules:

- View-model APIs must not mutate state.
- View-model APIs may compose data from multiple Cosmos containers and Blob artifacts.
- Resource APIs remain the source of truth.
- React screens should prefer view-model APIs over raw artifacts for primary rendering.
- Raw artifact JSON may be exposed only in developer/debug views.
...
The React frontend needs live updates for stage progress, Socrates persona activity, tool calls, evidence retrieval, quality gates, and selective re-reasoning. MVP should use **Server-Sent Events (SSE)** for one-way backend-to-browser updates. WebSocket may be added later only if bidirectional collaboration is required.

The canonical frontend pattern is:

```text
1. Load a snapshot view model, such as /pipeline/view or /socrates/view.
2. Load missed event history using /events?after_event_id=...
3. Open /events/stream with Last-Event-ID, if available.
4. Apply events idempotently to the client-side state.
5. On reconnect failure, reload snapshot and resume streaming.
```

### 11.1 List Session Events

```http
GET /api/v1/sessions/{session_id}/events
```

Returns durable JSON event history for replay, catch-up, and debugging.

#### Query Parameters

| Parameter | Type | Required | Description |
|---|---|---:|---|
| `after_event_id` | string | No | Return events after this event ID |
| `stage` | string | No | Filter by stage |
| `stage_run_id` | string | No | Filter by stage run |
  ...
  "items": [
    {
      ...
      "stage_run_id": "run_soc_001",
      "severity": "info",
      "message": "Socratic Review started.",
      "payload": {
        "depth": "standard",
        "personas": [
          "devils_advocate",
...
- Returns events in ascending timestamp order by default.
- Events should be durable for at least the active session lifetime.
- For MVP, events may be stored in the changelog container or an event-log collection.
- Event history is used to rehydrate the live reasoning trace after page reloads or dropped SSE connections.

### 11.2 Stream Session Events
...
data: {"event_id":"evt_002","event_type":"stage_progress","session_id":"sess_01jxyzabc123","stage":"socratic_review","stage_run_id":"run_soc_001","message":"FinOps Lead analysis completed.","percent":70,"timestamp":"2026-06-09T13:01:00Z"}

```

#### Behavior

- `id:` must equal the durable `event_id`.
- If `Last-Event-ID` is provided and history is available, the backend should replay missed events before streaming new ones.
- If replay is unavailable, the backend may return only new events; the frontend must reload snapshot APIs before subscribing.
- The stream may send heartbeat comments every 15–30 seconds.

```text
: heartbeat
```

- Client-side event handling must be idempotent.
- Streaming endpoints do not use the standard JSON response envelope.

### 11.3 Event Envelope

All persisted and streamed events should follow this shape.
...
|---|---:|---|
| `event_id` | Yes | Monotonic within a session or timestamp-sortable |
| `event_type` | Yes | Controlled event type |
| `session_id` | Yes | Parent session |
| `stage` | No | Present for stage-scoped events |
| `stage_run_id` | No | Present for execution-scoped events |
| `correlation_id` | No | Request/trace correlation |
| `severity` | Yes | `debug`, `info`, `warning`, `error` |
| `message` | Yes | Human-readable summary for live trace |
...
| `stage_started` | A stage run starts |
| `stage_progress` | A stage reports progress |
| `stage_completed` | A stage completes and its patch is applied |
| `stage_failed` | A stage fails |
| `stage_cancel_requested` | A running stage receives cancellation request |
| `quality_gate_updated` | A stage quality gate is evaluated or updated |
| `artifact_created` | A versioned artifact is created |
| `evidence_retrieved` | Retrieval returns evidence candidates |
| `tool_call_started` | A deterministic or external tool starts |
| `tool_call_completed` | A tool completes |
| `tool_call_failed` | A tool fails |
| `socrates_persona_started` | A Socrates persona starts analysis |
| `socrates_persona_completed` | A Socrates persona finishes analysis |
| `socrates_synthesis_started` | Synthesizer starts |
| `socrates_synthesis_completed` | Synthesizer completes |
| `evidence_audit_completed` | Evidence audit stage completes |
| `claim_validation_updated` | User validates/rejects an assumption claim |
| `change_detected` | Requirement/material change is registered |
| `impact_analysis_completed` | Dependency impact calculation completes |
| `rerun_plan_created` | Selective re-run plan is created |
| `rereasoning_started` | Selective re-reasoning begins |
| `rerun_stage_completed` | One impacted stage completes during re-reasoning |
| `diff_created` | Before/after diff is generated |
...
```json
{
  ...
  "stage_run_id": "run_soc_001",
  "severity": "info",
  "message": "Socratic Review completed with 5 persona findings and 3 assumptions requiring validation.",
  "payload": {
...
```json
{
  ...
  "stage_run_id": "run_soc_001",
  "severity": "info",
  "message": "Security Architect completed review.",
  "payload": {
    "persona": "security_architect",
    "summary": "Identified concerns around PCI-DSS scope, tokenization boundary, and private network isolation.",
    "finding_count": 3,
...
```json
{
  ...
  "severity": "error",
  "message": "Mermaid diagram could not be rendered after validation retries.",
  "payload": {
...
### 11.6 Reconnect and Replay Semantics

Frontend behavior:

```text
On page load:
  GET /sessions/{session_id}/pipeline/view
  GET /sessions/{session_id}/events?limit=100
  open /sessions/{session_id}/events/stream

On stream disconnect:
  remember last_event_id
  GET /sessions/{session_id}/events?after_event_id={last_event_id}
  reopen /sessions/{session_id}/events/stream with Last-Event-ID

If replay fails:
  reload relevant view model
  reopen stream
```
...
- Replayed events must preserve original event IDs and timestamps.
- Duplicate delivery is allowed; clients must de-duplicate by `event_id`.
- Missed event replay is best effort for MVP but should be implemented if events are persisted in changelog.
...
## 18. Frontend View Model APIs

These endpoints are optimized for the React/Next.js frontend described in `14-frontend-specification.md`.

They are **screen-ready read models**. They aggregate existing resource data from sessions, artifacts, claims, evidence, changelog, diffs, and stage executions. They do not replace lower-level resource APIs and they must not mutate state.

### 18.1 View-Model API Rules

1. React screens should use these endpoints for primary rendering.
2. The frontend should not parse raw `VersionedArtifact.content` to build hero screens.
3. Each view-model endpoint may include a `source_refs` array to help debugging and traceability.
4. Each response should include enough information for loading, empty, running, warning, failed, and completed states.
5. Each response should support demo-safe rendering even when some backend data is missing.
6. View-model APIs should return compact summaries by default and link to full artifact/resource APIs for drill-down.

### 18.2 Get Global Dashboard Summary

```http
GET /api/v1/dashboard/summary
```

Supports the Home / Architecture Command Center screen.

#### Query Parameters

| Parameter | Type | Required | Description |
|---|---|---:|---|
| `mode` | string | No | `demo` or `product`; default `product` |
| `limit` | integer | No | Number of recent sessions to include |
...
```json
{
  "metrics": {
    ...
    "running_sessions": 1,
    "failed_sessions": 0,
    "avg_quality_gate_score": 0.86,
    "avg_evidence_coverage": 0.78,
    "open_assumptions": 9,
  ...
  "recent_sessions": [
    {
      ...
      "title": "Fintech real-time fraud detection",
      "status": "completed",
      "current_stage": "final_evidence_audit",
      "active_version": 1,
      "updated_at": "2026-06-09T13:10:00Z"
    }
  ],
  "demo_readiness": {
    ...
    "creativity": "strong",
    "notes": [
      "Socrates Standard mode completed in latest demo session.",
      "Change Impact diff is available."
    ]
  }
}
```
...
P2 — polish / broader product.

### 18.3 Get Session Overview View

```http
GET /api/v1/sessions/{session_id}/overview
```

Supports session header, session landing, and cross-screen summary panels.
...
```json
{
  "session": {
    ...
    "status": "running",
    "current_stage": "socratic_review",
    "last_successful_stage": "options_generation",
    "active_version": 1,
    "created_at": "2026-06-09T12:45:00Z",
    "updated_at": "2026-06-09T13:10:00Z"
  },
  "summary": {
    ...
    "recommended_option_id": "OPT-A",
    "recommended_option_name": "Managed Azure Streaming Platform",
  ...
  "next_actions": [
    {
      ...
      "label": "Continue to Evidence Audit",
      "method": "POST",
      "href": "/api/v1/sessions/sess_01jxyzabc123/pipeline/run-next"
    }
  ],
  "latest_versions": {
    "requirements": 1,
...
P1 — needed for full workbench.

### 18.4 Get Pipeline View

```http
GET /api/v1/sessions/{session_id}/pipeline/view
```

Supports the Architecture Pipeline screen.

#### Query Parameters

| Parameter | Type | Required | Description |
|---|---|---:|---|
| `selected_stage` | string | No | Stage to include in the detail drawer |
| `include_events` | boolean | No | Include recent stage events |
| `event_limit` | integer | No | Default `20` |
  ...
  "stages": [
    {
      ...
      "progress": {
        "percent": 60,
        "message": "Security Architect and FinOps Lead completed."
      }
    }
  ],
  "selected_stage": {
    "stage": "socratic_review",
    "label": "Socratic Review",
    "summary": "Five personas are reviewing the architecture options.",
    "inputs": [
      {
        "label": "Options Matrix",
        "artifact_id": "art_options_001",
        "version": 1
      }
    ],
    "outputs": [],
    "quality_gate_detail": null,
    "artifact_refs": [],
    "recent_events": []
  }
}
...
P0 — required for React Phase 2 hero screen.

### 18.5 Get Requirements View

```http
GET /api/v1/sessions/{session_id}/requirements/view
```

Supports the Requirements Review screen.
...
```json
{
  "artifact": {
    ...
    "quality_gate": {
      "status": "passed_with_warnings",
      "warnings": [
        "Latency target is not explicit.",
  ...
  "completeness": {
    "score": 0.76,
    "missing_items": ["RTO/RPO", "data residency", "budget"],
    "blocking_items": []
  },
  "functional_requirements": [
    {
      "requirement_id": "FR-001",
      "description": "Ingest transaction events in real time.",
      "priority": "must",
      "source": "user",
      "status": "confirmed"
    }
  ...
  "traceability": [
    {
      ...
      "impacts_stages": ["options", "socratic_review", "hld"],
      "claim_ids": [],
      "evidence_ids": []
    }
  ]
...
### 18.6 Get Patterns View

```http
GET /api/v1/sessions/{session_id}/patterns/view
```

Supports the Pattern Explorer screen.
...
```json
{
  ...
  "primary_patterns": ["real_time_streaming"],
  ...
  "patterns": [
    {
      ...
      "category": "primary",
      "confidence": 0.9,
      "why_detected": "The requirement calls for continuous transaction ingestion and low-latency scoring.",
      "triggered_by_requirements": ["NFR-001", "FR-002"],
      "recommended_services": ["Azure Event Hubs", "Azure Stream Analytics", "Azure Functions"],
      "evidence_refs": ["ev_001"]
    }
  ],
  "design_implications": [
    "Backpressure and dead-letter handling must be explicit.",
    "Replay and audit path should be included."
  ],
  "evidence_summary": {
    "evidence_count": 4,
    "high_trust_count": 4
...
### 18.7 Get Options View

```http
GET /api/v1/sessions/{session_id}/options/view
```

Supports the Architecture Options board.
...
```json
{
  ...
  "options": [
    {
      "option_id": "OPT-A",
      "name": "Managed Azure Streaming Platform",
      "status": "recommended",
      "summary": "Azure-native managed streaming architecture optimized for MVP delivery and lower operational burden.",
      "services": ["Azure Event Hubs", "Azure Stream Analytics", "Azure Functions", "Cosmos DB"],
      "fit_score": 0.86,
      "scores": {
        "reliability": 0.8,
        "security": 0.85,
  ...
  "recommendation_summary": {
    "recommended_option_id": "OPT-A",
    "rationale": "Best balance of delivery speed, managed operations, and Azure-native fit for the initial 10K TPS scenario."
  }
}
```
...
### 18.8 Get Socrates View

```http
GET /api/v1/sessions/{session_id}/socrates/view
```

Supports the Socrates Reasoning Lab.
...
```json
{
  ...
  "decision_under_review": {
    ...
    "selected_option_name": "Managed Azure Streaming Platform",
    "requirements": ["10K TPS", "PCI-DSS", "99.95% availability"],
    "key_assumptions": []
  },
  "personas": [
    {
      ...
      "highest_severity": "high",
      "confidence": 0.82,
      "summary": "PCI-DSS scope and trust boundaries must be explicit.",
      "findings": [
        {
          "finding_id": "finding_001",
          "severity": "high",
          "title": "Tokenization boundary is not explicit",
          "description": "The design must clarify whether cardholder data reaches the streaming layer.",
          "linked_claim_ids": [],
          "linked_evidence_ids": []
        }
      ]
  ...
  "synthesis": {
    "recommended_option_id": "OPT-A",
    "recommendation": "Proceed with Option A for the initial 10K TPS scenario with explicit PCI-DSS controls.",
...
### 18.9 Get Evidence Explorer View

```http
GET /api/v1/sessions/{session_id}/evidence/view
```

Supports the Evidence & Claims Explorer screen.

#### Query Parameters

| Parameter | Type | Required | Description |
|---|---|---:|---|
| `stage` | string | No | Filter claims by stage |
| `type` | string | No | `fact`, `assumption`, `recommendation` |
| `status` | string | No | Supported, unsupported, contradiction, validation-needed |
| `trust` | string | No | Evidence trust filter |
| `requires_user_validation` | boolean | No | Filter assumptions requiring validation |
| `q` | string | No | Search claim/evidence text |
...
### 18.10 Get Artifact Package View

```http
GET /api/v1/sessions/{session_id}/artifacts/package-view
```

Supports Artifact Studio / Architecture Package.

#### Query Parameters

| Parameter | Type | Required | Description |
|---|---|---:|---|
| `active_tab` | string | No | `requirements`, `options`, `socrates`, `adr`, `hld`, `waf`, `evidence` |
| `version` | string | No | `latest` or explicit version |
...
```json
{
  ...
  "active_artifact": {
    ...
    "structured_sections": [
      {
        "title": "Context",
        "markdown": "The fintech platform needs real-time fraud scoring..."
      }
    ],
    "markdown": "# ADR-001: Use managed Azure streaming architecture
  ...
  "export_actions": [
    {
      ...
      "method": "POST",
      "href": "/api/v1/sessions/sess_01jxyzabc123/artifacts/export"
    }
  ]
}
```
...
```text
not_applicable | pending | rendered | failed | warning
```

Quality gate status and render status are independent. A Mermaid render failure may produce an artifact render warning without necessarily failing the full stage quality gate, depending on stage rules.

#### Priority

P0 — required for React Phase 2 hero screen.

### 18.11 Get Change Impact View

```http
GET /api/v1/sessions/{session_id}/changes/{change_event_id}/impact-view
```

Supports Change Impact Studio.
...
```json
{
  "change_event": {
    ...
    "change_categories": ["scale", "availability", "region"],
    "created_at": "2026-06-09T13:30:00Z"
  },
  "impact": {
    "impacted_stages": ["options", "socratic_review", "adr", "hld", "waf_review", "final_evidence_audit"],
    "stable_stages": ["intake", "requirements", "pattern_detection"],
    "rationale": [
      {
        "stage": "options",
        "reason": "Scale and active-active topology affect viable architecture options."
      }
    ]
  },
  "rerun_plan": {
    "status": "ready",
    "ordered_stages": ["options", "socratic_review", "adr", "hld", "waf_review", "final_evidence_audit"],
  ...
  "diffs": [
    {
      ...
      "before_version": 1,
      "after_version": 2,
      "summary": "The HLD changed from single-region streaming to multi-region active-active streaming.",
      "changes": []
    }
  ]
}
...
### 18.12 Legacy Viewer APIs

The following endpoints may remain for local debugging, Streamlit legacy compatibility, or admin/developer views:
...
- These are not the primary React frontend contract.
- New React screens should use `/overview`, `/pipeline/view`, `/artifacts/package-view`, and other view-model endpoints.
- Do not add new product features only to the legacy viewer layer.
...
|---|---|
| `dashboard.py` | Cross-session dashboard summary |
| `frontend_views.py` | `/overview`, `/pipeline/view`, `/requirements/view`, `/patterns/view`, `/options/view`, `/socrates/view`, `/evidence/view`, `/artifacts/package-view`, `/changes/{id}/impact-view` |
| `events.py` | Event history and SSE stream |
| `viewer.py` | Legacy/debug artifact viewer endpoints |
...
events_router = APIRouter(prefix="/sessions/{session_id}/events", tags=["events"])
...
async def list_session_events(
    ...
    \"\"\"Return durable JSON event history.\"\"\"
    ...
...
### 24.2 Load Snapshot and Start Event Stream

React should load a snapshot before opening the SSE stream.

```http
GET /api/v1/sessions/{session_id}/overview
GET /api/v1/sessions/{session_id}/pipeline/view
GET /api/v1/sessions/{session_id}/events?limit=100
GET /api/v1/sessions/{session_id}/events/stream
```
...
Legacy Streamlit/debug flow may continue to use:

```http
GET /api/v1/sessions/{session_id}/dashboard
GET /api/v1/sessions/{session_id}/viewer/artifacts
GET /api/v1/sessions/{session_id}/viewer/diagrams
```
...
| Session overview view | `< 500 ms` locally / low data volume |
| View-model APIs | `< 700 ms` locally / low data volume |
| Dashboard summary | `< 1 s` locally / low data volume |
| Stage start APIs | `< 1 s` to return accepted response |
| Stage execution | Async; may take seconds to minutes |
| Event history query | `< 500 ms` for recent events |
...
- If streaming disconnects, the frontend must recover through `/events?after_event_id=...` and snapshot view APIs.
- View-model APIs should tolerate partially available data and return explicit `status`, `warnings`, or empty-state structures instead of failing when one optional artifact is missing.
- Event delivery may be at-least-once; frontend clients must de-duplicate by `event_id`.
...
- Expose only structured reasoning trace: stage events, persona summaries, findings, claims, evidence, tool calls, and quality gates.
...
Every tool call, retrieval call, stage transition, Socrates persona execution, evidence audit, and re-reasoning event should be correlated to:

- `session_id`
- `stage_run_id`
- `event_id`, if user-visible
- `correlation_id`

## 26. MVP Endpoint Checklist

The following endpoints are required or recommended for the React/Next.js MVP.

### 26.1 P0 — Required for React hero screens

| Endpoint | Required |
|---|---:|
| `POST /sessions` | Yes |
| `GET /sessions/{session_id}` | Yes |
| `GET /sessions/{session_id}/overview` | Yes |
| `GET /sessions/{session_id}/pipeline` | Yes |
| `GET /sessions/{session_id}/pipeline/view` | Yes |
| `POST /sessions/{session_id}/pipeline/run` | Yes |
| `POST /sessions/{session_id}/pipeline/run-next` | Yes |
| `GET /sessions/{session_id}/events` | Yes |
| `GET /sessions/{session_id}/events/stream` | Yes |
| `GET /sessions/{session_id}/socrates/view` | Yes |
| `GET /sessions/{session_id}/evidence/view` | Yes |
| `GET /sessions/{session_id}/artifacts/package-view` | Yes |
| `POST /sessions/{session_id}/claims/{claim_id}/validate` | Yes |
| `POST /sessions/{session_id}/changes` | Yes |
| `GET /sessions/{session_id}/changes/{change_event_id}/impact-view` | Yes |
| `POST /sessions/{session_id}/changes/{change_event_id}/rereason` | Yes |
| `GET /sessions/{session_id}/diffs` | Yes |
| `GET /health` | Yes |
| `GET /health/ready` | Yes |

### 26.2 P1 — Needed for full workbench

| Endpoint | Required |
|---|---:|
| `GET /sessions` | Yes |
| `GET /sessions/{session_id}/requirements/view` | Recommended |
| `GET /sessions/{session_id}/options/view` | Recommended |
...
| `GET /sessions/{session_id}/claims/{claim_id}` | Recommended |
| `GET /sessions/{session_id}/evidence` | Recommended |
| `GET /sessions/{session_id}/audits/evidence/latest` | Recommended |

### 26.3 P2 — Polish / broader product

| Endpoint | Required |
|---|---:|
| `GET /dashboard/summary` | Recommended |
| `GET /sessions/{session_id}/patterns/view` | Recommended |
| `GET /sessions/{session_id}/artifacts/export` or `POST /sessions/{session_id}/artifacts/export` | Optional |
| `GET /sessions/{session_id}/viewer/artifacts` | Legacy / optional |
| `GET /sessions/{session_id}/viewer/diagrams` | Legacy / optional |
| `POST /admin/debug/retrieval` | Dev only |
...
The API layer is ready for the React MVP when:

1. A user can create a session from a business need.
2. The frontend can retrieve session overview and pipeline view models.
...
6. Event history can be queried through `/events`.
7. Live events can be streamed through `/events/stream`.
8. Stream reconnect can recover through `Last-Event-ID` or snapshot reload.
9. Artifacts can be retrieved by stage and latest version.
10. Artifact Studio can render from `/artifacts/package-view` without parsing raw artifact JSON.
11. Socrates Reasoning Lab can render persona status, persona findings, synthesis, confidence, blind spots, and assumptions from `/socrates/view`.
12. Evidence Explorer can render claims, evidence coverage, unsupported assumptions, contradictions, and selected claim details from `/evidence/view`.
13. Claims and evidence can be listed and linked through resource APIs.
14. User can validate or reject assumptions through `/claims/{claim_id}/validate`.
15. Evidence audit output is retrievable.
16. A requirement change can be submitted.
17. Impacted and stable stages are returned.
18. Change Impact Studio can render from `/changes/{change_event_id}/impact-view`.
19. Selective re-reasoning creates new artifact versions.
20. Before/after diffs can be retrieved.
21. Version conflicts and idempotency conflicts return clear errors.
22. Health and readiness endpoints work.
23. Legacy Streamlit/viewer endpoints, if present, do not contradict the React view-model APIs.
...
For the React MVP, prioritize stable session creation, pipeline execution, event history/SSE streaming, screen-ready view-model APIs, artifact retrieval, evidence visibility, and re-reasoning APIs. More advanced APIs such as artifact export, admin retrieval debugging, full RBAC, and manual artifact editing can be added later.
