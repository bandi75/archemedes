...
const originalNeed = "Design a real-time fraud detection platform on Azure for a fintech processing 10K TPS with PCI-DSS constraints and 99.95% availability.";

const refinedNeed = "Design an Azure-first real-time fraud detection platform for fintech card transactions, with PCI-DSS boundaries, high-throughput ingestion, evidence-backed service choices, and explicit availability targets.";
...
function IntakePageContent() {
  ...
  const [sessionId, setSessionId] = useState<string | null>(() => {
    ...
    return searchParams.get("sessionId") ?? window.localStorage.getItem("archimedes.activeSessionId");
  });
  ...
  async function resolveActiveSessionId() {
    ...
    if (urlSessionId) {
      ...
      window.localStorage.setItem("archimedes.activeSessionId", urlSessionId);
      return urlSessionId;
    }

    const storedSessionId = window.localStorage.getItem("archimedes.activeSessionId");
    if (storedSessionId) {
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-lg border border-border bg-panel p-4 shadow-panel">
          <div className="flex flex-wrap gap-2">
            <button className="inline-flex h-9 items-center rounded-md border border-border bg-surface px-3 text-sm font-medium text-ink" type="button">
              Edit business need
            </button>
            <button className="inline-flex h-9 items-center rounded-md bg-accent px-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50" disabled={submitting} onClick={submitAnswers} type="button">
              {submitting ? "Submitting..." : "Submit clarification answers"}
            </button>
            <button className="inline-flex h-9 items-center rounded-md border border-border bg-surface px-3 text-sm font-medium text-ink disabled:cursor-not-allowed disabled:opacity-50" disabled={hasOpenQuestions} onClick={markReady} title={hasOpenQuestions ? "Answer required clarifications first." : "Ready for requirements extraction."} type="button">
              Mark ready for requirements
            </button>
            <Link className="inline-flex h-9 items-center rounded-md border border-border bg-surface px-3 text-sm font-medium text-ink" href="/pipeline">
              Back to pipeline
            </Link>
          </div>
          <p className="mt-3 text-sm text-ink-muted" role="status">{status}</p>
        </section>
        ...
        <section className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
          <main className="space-y-5">
              ...
              <div className="mt-4 space-y-3">
                {openQuestions.map((question, index) => (
                  <div key={question} className="rounded-md border border-border bg-surface p-3">
                    <p className="text-sm font-medium text-ink">{index + 1}. {question}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="rounded-lg border border-border bg-panel p-5 shadow-panel">
              ...
              <label className="mt-4 block text-sm font-medium text-ink" htmlFor="clarification-response">Clarification answers</label>
              <textarea
                ...
                value={answers}
              />
              <button className="mt-4 inline-flex h-10 items-center rounded-md bg-accent px-4 text-sm font-semibold text-white transition hover:bg-ink disabled:cursor-not-allowed disabled:opacity-50" disabled={submitting} onClick={submitAnswers} type="button">
            ...
            <section className="rounded-lg border border-border bg-panel p-5 shadow-panel">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {[
                  ...
                  <p key={note} className="rounded-md border border-border bg-surface p-3 text-sm leading-6 text-ink-muted">{note}</p>
                ))}
              </div>
            </section>
          </main>
            ...
            <div className="rounded-lg border border-border bg-panel p-5 shadow-panel">
              ...
              <div className="mt-4 space-y-3">
                {checklist.map(([label, state, variant]) => (
                  <div key={label} className="flex items-center justify-between gap-3 rounded-md border border-border bg-surface p-3">
                ...
                <div className="flex items-center justify-between gap-3 rounded-md border border-border bg-surface p-3">
                  ...
                  <StatusBadge variant={submitted ? "success" : "warning"}>{submitted ? "submitted" : "needed"}</StatusBadge>
                </div>
                <div className="flex items-center justify-between gap-3 rounded-md border border-border bg-surface p-3">
    ...
    <AppShell>
      <div className="space-y-6">
        <PageHeader
          breadcrumb={["Session Workspace", "Intake"]}
          title="Intake Stage Detail"
          description="Loading intake context..."
          badges={[{ label: "Stage 1", variant: "info" }]}
        />
      </div>
    </AppShell>
  ...
  return (
    <section className="rounded-lg border border-border bg-panel px-5 py-4 shadow-panel">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        ...
        <StatusBadge variant={ready ? "success" : "warning"}>{ready ? "Pipeline status: Intake ready" : "Pipeline status: Waiting for intake readiness"}</StatusBadge>
      </div>
    </section>
  );
}
...
function InfoCard({ title, body, accent = false }: { title: string; body: string; accent?: boolean }) {
  return (
    <section className={accent ? "rounded-lg border border-accent/20 bg-accent/5 p-5 shadow-panel" : "rounded-lg border border-border bg-panel p-5 shadow-panel"}>
